...
function creating_and_solving_model(DIFFERENT_VAR, NB_BLOCKS, DIFFERENT_BLOCKS, block_var,CLIQUE_TREE, CONSTRAINTS, MATRICES, constants_per_constraint,my_solver, objscale, time_limit)
    #initialize
    ...
    println("Scaling objective function : factor or 10^-6")
...
function sdp_model_solver_to_specify(cliques,
                                     cliquetree,
                                     path_opf_ctr,
                                     path_opf_mat,
                                     objscale;
    ...
    #read data files
    @debug "    Reading data"
    my_timer=@elapsed (BLOCKS, CLIQUE_TREE, CONSTRAINTS, MATRICES_with_CONSTANTS) = read_data(cliques, cliquetree, path_opf_ctr, path_opf_mat)
    @printf("%-35s%10.6f s\n", "read_data", my_timer)
    #treat constants
    @debug "    Treating constants"
